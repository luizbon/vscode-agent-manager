...
suite("AgentDiscovery Test Suite", () => {
  ...
  test("searchAgents handles failures gracefully", async () => {
    sandbox
      .stub(GitService.prototype, "cloneOrPullRepo")
      .rejects(new Error("Git failure"));
    ...
    const agents = await agentDiscovery.searchAgents([
  ...
  test("fetchAgentsFromRepo processes agent files", async () => {
    ...
    sandbox
      .stub(GitService.prototype, "getRepoDirName")
      .returns("mock_repo_dir");
...
`;
    ...
    const agents = await agentDiscovery.fetchAgentsFromRepo(
